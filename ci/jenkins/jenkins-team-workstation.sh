...
status() { positive 'build number' "$1"; local s; s="$(state "$1")" || die "Build #$1 is not available yet. Retry shortly."; echo "Build #$1: $s"; [[ "$s" == SUCCESS ]]; }
follow() { positive 'build number' "$1"; while :; do local s; if ! s="$(state "$1")"; then echo "Build #$1 is not available yet; retrying." >&2; sleep 2; continue; fi; [[ "$s" == BUILDING ]] || { echo "Build #$1: $s"; [[ "$s" == SUCCESS ]]; return; }; sleep 5; done; }
